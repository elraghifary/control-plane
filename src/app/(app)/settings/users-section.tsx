            ...
            <>
              <div className="space-y-3 px-5 py-5">
                <p className="text-sm text-muted-foreground">
                  Share this link with <span className="font-mono text-foreground">{email.trim().toLowerCase()}</span>. It expires in 7 days.
                </p>
    ...
    <div className="flex flex-col gap-3 rounded-lg border border-border/70 bg-card/40 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0 space-y-0.5">
        ...
        <p className="truncate text-xs text-muted-foreground">
          <span className="font-mono">{user.githubLogin}</span> · joined {formatJoined(user.createdAt)}
        </p>
      </div>
      {!isSelf && (
