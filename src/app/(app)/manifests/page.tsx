...
import { listManifestPagesAction } from "./actions";
import { CreateManifestDialog } from "@/components/manifests/create-manifest-dialog";
...
export default async function ManifestsPage({
  ...
  try {
    ...
    const [repos, docsResult] = await Promise.all([
      ...
      listManifestPagesAction(),
    ]);
    repositories = repos;
    if (!docsResult.ok) {
      errorMsg = docsResult.error ?? "Failed to load manifest documents";
    } else {
    ...
    errorMsg = e instanceof Error ? e.message : "Failed to load manifest documents";
  }

  if (errorMsg) {
    return <ErrorState title="Failed to load manifests" description={errorMsg} />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <ManifestList docs={pageDocs} repositories={repositories} />
